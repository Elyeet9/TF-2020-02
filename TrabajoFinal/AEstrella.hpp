...
public:
	AEstrella(Laberinto^ laberinto, short corruptX, short corruptY, short reyX, short reyY) {
		...
		PrimerNodo = &nodos[corruptY / CAS_ANCHO * ancho + corruptX / CAS_ANCHO];
	...
	short get_new_dx(short corruptX, short velocidad) {
		if (UltimoNodo != nullptr)
		{
			...
			if (corruptX < p->x * CAS_ANCHO)
				return velocidad;
			else if (corruptX > p->x * CAS_ANCHO) return velocidad * -1;
		}
	}

	short get_new_dy(short corruptY, short velocidad) {
		if (UltimoNodo != nullptr)
		{
			aNodo* p = UltimoNodo;
			while (p->padre->padre != nullptr)
			...
			if (corruptY > p->y * CAS_ANCHO)
				return velocidad * -1;
			else if (corruptY < p->y * CAS_ANCHO) return velocidad;
		}
	}
