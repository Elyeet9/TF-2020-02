...
public:
		...
		timer(0), timer_limite(timer_limite*10), timer_ganar(0), timer_negociacion(0), n(n), negociado(false)
	{
		...
		this->aliados = gcnew CArregloAliados("img\\aliado.png", n, area_dibujo.Width, area_dibujo.Height, laberinto, this->rey->getX(), this->rey->getY());
	...
	void mover_rey(bool cam, Keys tecla) { 
		if (cam) {
			switch (tecla)
			{
			case Keys::W: this->rey->set_direccion(this->rey->get_dx(), -10); break;
			case Keys::A: this->rey->set_direccion(-10, this->rey->get_dy()); break;
			case Keys::S: this->rey->set_direccion(this->rey->get_dx(), 10); break;
			case Keys::D: this->rey->set_direccion(10, this->rey->get_dy()); break;
			}
		}
		else {
			switch (tecla)
			{
			case Keys::W: this->rey->set_direccion(this->rey->get_dx(), 0); break;
			case Keys::A: this->rey->set_direccion(0, this->rey->get_dy()); break;
			case Keys::S: this->rey->set_direccion(this->rey->get_dx(), 0); break;
			case Keys::D: this->rey->set_direccion(0, this->rey->get_dy()); break;
			}
		}
	}
